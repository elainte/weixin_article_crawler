...
class WeixinOperate():
    ...
    def search_gzh(self, nickname):
        ...
        print('键入主界面:'+nickname)
        time.sleep(2)
        # 上拉
        self.oap.roll(0, 500)
        print('上拉 找到历史消息:'+nickname)
        time.sleep(2)
        return 0
    ...
    def click_a_message(self, args=2):
        ...
        # 获取界面文章标题消息
        if args == 1:
            corp = CROP_RANGE['PROFILE_MESSAGE_LIST']
        elif args == 2:
            corp = CROP_RANGE['MESSAGE_LIST']
        ...
        pos = [loc['left'], loc['top'], loc['left'] +
               loc['width'], loc['top']+loc['height']]
        self.oap.tap(pos)
        # 等待页面加载完毕
        time.sleep(5)
        self.oap.roll(0, 500)
    ...
    def check_comments(self):
        ...
        ui_words_str = self.vc.get_ui_words(
            location=False, in_str=True, crop=CROP_RANGE['LEAVE_MSG_BOTTOM'])
        # 如果暂无评论点击了留言按钮
